...
A comparison of all methods included in this paper can be run with `_compare.py`.-1 The
comparison in Appendix B can be run with `_compare_taskID.py`-2, and Figure C.1 can be recreated with `_compare_replay.py`-3.
...
`_compare_time.py`-4. This includes a comparison of the time these methods take to train (Figures 4 and 5).
